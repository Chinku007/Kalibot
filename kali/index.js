...
fs.writeFileSync("./Kali_Config.json", JSON.stringify(global.fca.data, null, "\t"))
  return process.exit(1)
}
try {
      var langfile = JSON.parse(fs.readFileSync(__dirname + "/languages/languages.json", 'utf-8'));
var lang
    switch (require("../../Kali_Config.json").languages) {
      ...
      case "jp": lang = langfile.jp.index;
        break;
      default: {
      log.warn("warn", "Only 4 Kinds of Languages ​​EN, VI, TH & JP Currently Supported, Self Fix By Finding And Deleting Kali_Config.json File")
      process.exit(0)
      }
    }
  } catch(e) {
...
global.fca = new Object({
     ObjFcaConfig: require("../../Kali_Config.json"),
     languages: lang
})
...
axios.get(`https://uptime-api.0xanupx0.repl.co/uptime?link=${UrlRepl}`)
		logger(global.fca.languages.uptime + UrlRepl, '[ KALI ]');
	}
	catch {
		logger(global.fca.languages.erroUptime, '[ KALI ]');
  }
}
...
function buildAPI(globalOptions, html, jar) {
    ...
    logger(`${global.fca.languages.loginSu}${userID}`, "[ KALI ]");
    ...
    if (oldFBMQTTMatch) {
        ...
        logger(`${global.fca.languages.region}${region}`, "[ KALI ]");
    } else {
...
logger(global.fca.languages.login, "[ KALI ]");
        return utils
            ...
            .then(function(res) {
                ...
                if (headers.location.indexOf('https://www.facebook.com/checkpoint/') > -1) {
                    logger(global.fca.languages.errorFa, "[ KALI ]");
                                            ...
                                            utils
                                                ...
                                                .then(res => {
                                                    try {
                                                        ...
                                                        clearInterval(checkVerified);
                                                        logger(global.fca.languages.okweb, "[ FCA - KALI ]");
                                                        if (callback === prCallback) {
                                                            callback = function(err, api) {
                                                                if (err) return prReject(err);
                                                                return prResolve(api);
...
require('dotenv').config({ path: './../.env' });
                ...
                var ans = makeid(49)
                    process.env["FBKEY"] = ans;
                        fs.writeFile('./../.env', `FBKEY=${ans}`, function (err) {
                            if (err) {
                            logger(global.fca.languages.errorEnv, "[ KALI ]");
                    }
                else logger(global.fca.languages.env,"[ KALI ]")
        }); 
    }
    catch (e) {
        console.log(e);
        logger(global.fca.languages.errorPassRandom, "[ KALI ]");
    }
}
    ...
    if (process.env['FBKEY']) {
        try {
            ...
            if (appState.includes('[')) {
                logger(global.fca.languages.appstateDec, '[ KALI ]');
            } else {
                try {
                    appState = JSON.parse(appState);
                    var StateCrypt = require('./StateCrypt');
                    appState = StateCrypt.decryptState(appState, process.env['FBKEY']);
                    logger(global.fca.languages.appstate, '[ KALI ]');
                }
                catch (e) {
                    logger(global.fca.languages.appstateError, '[ KALI ]');
                }
            }
        }
        catch (e) {
    ...
    try {
        ...
        try {
            ...
            return logger(global.fca.languages.appstateError, '[ KALI ]')
        }
    }
      }
    try { 
    ...
    mainPromise
        .then(function() {
            logger(global.fca.languages.oklogin, "[ KALI ]");
                    //!---------- Auto Check, Update START -----------------!//
                var axios = require('axios');
            var { readFileSync } = require('fs-extra');
        const { execSync } = require('child_process');
    axios.get('https://raw.githubusercontent.com/chinku007/kalibot/main/kali/package.json').then(async (res) => {
        const localbrand = JSON.parse(readFileSync('./node_modules/kali/package.json')).version;
            if (localbrand != res.data.version) {
                        ...
                        log.warn("UPDATE > ",`${global.fca.languages.autoUpdate}`);
                            try {
                                execSync('npm install kali@latest', { stdio: 'inherit' });
                                logger(global.fca.languages.okUpdate,"UPDATE")
                                logger(global.fca.languages.restart, '[ KALI ]');
                    ...
                    logger(`${global.fca.languages.checkVersion}` + localbrand + ' !', "[ KALI ]");      
                         logger(global.fca.languages.chucAdmin, "[ KALI ]");
